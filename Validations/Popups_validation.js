...
/*----------------------VALIDATIONS-NINJA-FORMS------------------------*/
    /*VALIDATION FOR NAMES, INSTITUTION NAMES, CITY */
    $(".asf-email, .donate, .va-apply, .va-email, .epics-apply, .epics-email ,.acon-email ,#register-leadership, .leadership-email, .journal-email, .iieecp-email, .eer-email, .slc-email, .rtec-register-button, .email-us-consortium, #iieecp-apply, .eer-apply, .slc-apply").click(function(){
  		...
  		$("#nf-field-198, #nf-field-116, #nf-field-79, #nf-field-68, #nf-field-86").change(function(){
  			var str = $(this).val();
    		var reg = /(^[+])([\d]{2})([-]{1})([\d]{10})/g;	//FORMAT => +CC-DDDDDDDDDDD
		...
		$('.nf-element').change(function(){
  			var str = $(this).val();
  			var res = str.match(/(["]|['])/g);	//CHECKING FOR QUOTES TO AVOID SQL INJECTION ATTACK
  			if(res!=null){
      			$(this).val("");
